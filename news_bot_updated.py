...
async def add_user(user_id, user_name):
    try:
        ...
        if not existing_user:
            cursor.execute("INSERT INTO users (user_id, user_name, notifications) VALUES (%s, %s, FALSE)",
                           (user_id, user_name))
            connection.commit()
...
def get_notification_keyboard():
    notification = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text='✅ Да', callback_data='subscribe'),
         InlineKeyboardButton(text='❌ Нет', callback_data='unsubscribe')]
    ])
    return notification
...
async def parse_news_informburo():
    ...
    for block in soup.find_all('li', class_='uk-grid uk-grid-small uk-margin-remove-top'):
        ...
        if not result:
            ...
            cursor.execute(
                "INSERT INTO informburo_news (title, photo, time, mark, link, content) VALUES (%s, %s, %s, %s, %s, %s) RETURNING id",
                (article_title, 'https://informburo.kz' + article_photo, article_time, article_mark, article_url,
                 article_content))
...
async def parse_news_nur():
    ...
    for article in soup.find_all("a", class_="article-preview-category__content"):
        ...
        if not result:
            # Создаем запись о статье в базе данных
            cursor.execute(
                "INSERT INTO nur_news (title, photo, time, category, link, content) VALUES (%s, %s, %s, %s, %s, %s) RETURNING id",
...
async def get_news_informburo(message: types.Message, page_number: int = 1):
    ...
    cursor.execute("SELECT id, title, photo, time, mark, link FROM informburo_news ORDER BY id ASC OFFSET %s LIMIT %s",
